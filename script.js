...
canvas.addEventListener(
	...
	function(event) {
		...
		if (
			coords.x >= onePlayer.x &&
			coords.y <= onePlayer.y + onePlayer.h &&
			coords.x <= onePlayer.x + onePlayer.w &&
			coords.y >= onePlayer.y
		) {
			...
			coords.x >= twoPlayer.x &&
			coords.y <= twoPlayer.y + twoPlayer.h &&
			coords.x <= twoPlayer.x + twoPlayer.w &&
			coords.y >= twoPlayer.y
		) {
			var player = new Paddle('human', 20, 100, 20, 100); // Left Paddle
		...
		// If the Game is over & the restart button has been clicked
			...
			if (
				coords.x >= restart.x &&
				coords.y <= restart.y + restart.h &&
				coords.x <= restart.x + restart.w &&
				coords.y >= restart.y
			) {
				animate(main);
